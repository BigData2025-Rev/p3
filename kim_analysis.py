...
df = df.filter(df.summary_level == 160)
...
df2000 = df2000.toDF(*["name" if c == "name" else f"{c}_2000" for c in df2000.columns])
df2000.show()

df2020 = df.filter(df.year == 2020)
df2020 = df2020.toDF(*["name" if c == "name" else f"{c}_2020" for c in df2020.columns])
df2020.show()

finaldf = df2000.join(df2020, on="name", how="inner")
...
finaldf = finaldf.filter(finaldf.minority_race_2000 == finaldf.majority_race_2020)
finaldf.show()
